...
#### 无单独的数据分区
  ...
  - 具体步骤：  
    - 保持当前系统和数据目录不变，将新磁盘插入系统，将新磁盘(假设为``/dev/nvme0n1``固态盘)转换为``lvm``，假设配置好后的逻辑卷路径为```/dev/mapper/LVM-DB_DATA```，参考 [单独磁盘创建LVM](#单独分区创建LVM)
    
    - 以下步骤，在用户**完成** [单独磁盘创建LVM](#单独分区创建LVM) 后，方可参考执行
